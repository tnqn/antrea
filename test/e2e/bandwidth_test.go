...
func TestPodTrafficShaping(t *testing.T) {
	...
	tests := []struct {
		...
		// Use a range to allow a certain deviation.
	...
	}{
		{
			...
			expectedMinBandwidth:   90,
			expectedMaxBandwidth:   110,
		},
		{
			name:                   "limited by ingress bandwidth",
			clientEgressBandwidth:  300,
			serverIngressBandwidth: 200,
			expectedMinBandwidth:   180,
			expectedMaxBandwidth:   220,
		},
	}
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Greater(t, actualBandwidth, float64(tt.expectedMinBandwidth))
			assert.Less(t, actualBandwidth, float64(tt.expectedMaxBandwidth))
		})
	}
}
