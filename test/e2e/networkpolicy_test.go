...
func TestNetworkPolicyStats(t *testing.T) {
	...
	if err := data.mutateAntreaConfigMap(func(data map[string]string) {
		...
		antreaControllerConf = strings.Replace(antreaControllerConf, "#  NetworkPolicyMetrics: false", "  NetworkPolicyMetrics: true", 1)
		data["antrea-controller.conf"] = antreaControllerConf
		antreaAgentConf, _ := data["antrea-agent.conf"]
		antreaAgentConf = strings.Replace(antreaAgentConf, "#  NetworkPolicyMetrics: false", "  NetworkPolicyMetrics: true", 1)
		data["antrea-agent.conf"] = antreaAgentConf
	}, true, true); err != nil {
		t.Fatalf("Failed to enable NetworkPolicyMetrics feature: %v", err)
	}
	...
	np1, err := data.createNetworkPolicy("test-networkpolicy-ingress", spec1)
	if err != nil {
	...
	np2, err := data.createNetworkPolicy("test-networkpolicy-egress", spec2)
	if err != nil {
		t.Fatalf("Error when creating network policy: %v", err)
	}
	defer func() {
	...
	// Wait for 5 seconds before starting connections to count the metrics accurately.
	...
	if err := wait.Poll(5*time.Second, defaultTimeout, func() (bool, error) {
		for _, np := range []string{"test-networkpolicy-ingress", "test-networkpolicy-egress"} {
			metric, err := data.crdClient.MetricsV1alpha1().NetworkPolicyMetricses(testNamespace).Get(context.TODO(), np, metav1.GetOptions{})
			if err != nil {
				return false, err
			}
			t.Logf("Got NetworkPolicy metric: %v", metric)
			if metric.Stats.Sessions != int64(sessions) {
				return false, nil
			}
			if metric.Stats.Packets < metric.Stats.Sessions || metric.Stats.Bytes < metric.Stats.Sessions {
				return false, fmt.Errorf("Neither 'Packets' nor 'Bytes' should be smaller than 'Sessions'")
			}
		}
		return true, nil
	}); err != nil {
		t.Fatalf("Error when waiting for NetworkPolicy metrics: %v", err)
	}
}
