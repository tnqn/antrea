...
func NewNetworkPolicyController(kubeClient clientset.Interface,
	...
	n.groupEntityIndex.AddEventHandler(appliedToGroupType, n.enqueueAppliedToGroup)
	n.groupEntityIndex.AddEventHandler(addressGroupType, n.enqueueAddressGroup)
	n.groupEntityIndex.AddEventHandler(clusterGroupType, n.enqueueInternalGroup)
...
func (n *NetworkPolicyController) createAppliedToGroup(npNsName string, pSel, nSel, eSel *metav1.LabelSelector) string {
	...
	n.groupEntityIndex.AddGroup(appliedToGroupType, newAppliedToGroup.Name, groupSelector)
...
func (n *NetworkPolicyController) createAddressGroup(namespace string, podSelector, nsSelector, eeSelector *metav1.LabelSelector) string {
	...
	n.groupEntityIndex.AddGroup(addressGroupType, addressGroup.Name, groupSelector)
	return normalizedUID
}
...
func (n *NetworkPolicyController) deleteDereferencedAddressGroups(internalNP *antreatypes.NetworkPolicy) {
	...
	for _, key := range addressGroupKeys {
		...
		if len(nps) == 0 {
			...
			n.groupEntityIndex.DeleteGroup(addressGroupType, key)
		}
	}
}
...
func (n *NetworkPolicyController) deleteDereferencedAppliedToGroup(key string) {
	...
	if len(nps) == 0 {
		...
		n.groupEntityIndex.DeleteGroup(appliedToGroupType, key)
	}
}
...
func (n *NetworkPolicyController) Run(stopCh <-chan struct{}) {
	...
	cacheSyncs := []cache.InformerSynced{n.networkPolicyListerSynced}
...
func (n *NetworkPolicyController) populateAddressGroupMemberSet(g *antreatypes.AddressGroup) controlplane.GroupMemberSet {
	...
	if found {
		...
		pods, externalEntities = n.groupEntityIndex.GetEntities(clusterGroupType, g.Name)
	} else {
		pods, externalEntities = n.groupEntityIndex.GetEntities(addressGroupType, g.Name)
	}
...
func (n *NetworkPolicyController) getAppliedToWorkloads(g *antreatypes.AppliedToGroup) ([]*v1.Pod, []*v1alpha2.ExternalEntity) {
	...
	_, found, _ := n.internalGroupStore.Get(g.Name)
	if found {
		return n.groupEntityIndex.GetEntities(clusterGroupType, g.Name)
	}
	return n.groupEntityIndex.GetEntities(appliedToGroupType, g.Name)
}
