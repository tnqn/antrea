...
func TestGetAssociatedGroups(t *testing.T) {
	...
	for i := range testPods {
		npc.groupEntityIndex.AddPod(testPods[i])
	}
	for j := range externalEntities {
		npc.groupEntityIndex.AddExternalEntity(externalEntities[j])
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for i, g := range tt.existingGroups {
				npc.internalGroupStore.Create(&tt.existingGroups[i])
				npc.groupEntityIndex.AddGroup(clusterGroupType, g.Name, g.Selector)
			}
...
func TestGetGroupMembers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			npc.groupEntityIndex.AddGroup(clusterGroupType, tt.group.Name, tt.group.Selector)
