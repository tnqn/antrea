...
func TestAddressGroupWithNodeSelector(t *testing.T) {
	...
	agID := c.createAddressGroup("uid", "", nil, nil, nil, &nodeSelectorA)
...
func TestSyncInternalNetworkPolicy(t *testing.T) {
	...
	checkQueueItemExistence(t, c.addressGroupQueue)
...
func TestGroupReferences(t *testing.T) {
	r := newGroupReferences()

	r.reference("groupA", "uid1")
	assert.Equal(t, sets.NewString("uid1"), r.references("groupA"))
	assert.Equal(t, sets.NewString("groupA"), r.referencedBy("uid1"))

	r.reference("groupA", "uid2")
	assert.Equal(t, sets.NewString("uid1", "uid2"), r.references("groupA"))
	assert.Equal(t, sets.NewString("groupA"), r.referencedBy("uid1"))
	assert.Equal(t, sets.NewString("groupA"), r.referencedBy("uid2"))

	r.reference("groupB", "uid1")
	assert.Equal(t, sets.NewString("uid1", "uid2"), r.references("groupA"))
	assert.Equal(t, sets.NewString("uid1"), r.references("groupB"))
	assert.Equal(t, sets.NewString("groupA", "groupB"), r.referencedBy("uid1"))
	assert.Equal(t, sets.NewString("groupA"), r.referencedBy("uid2"))

	r.dereference("groupA", "uid1")
	assert.Equal(t, sets.NewString("uid2"), r.references("groupA"))
	assert.Equal(t, sets.NewString("uid1"), r.references("groupB"))
	assert.Equal(t, sets.NewString("groupB"), r.referencedBy("uid1"))
	assert.Equal(t, sets.NewString("groupA"), r.referencedBy("uid2"))

	r.dereference("groupA", "uid2")
	assert.Equal(t, sets.NewString(), r.references("groupA"))
	assert.Equal(t, sets.NewString("uid1"), r.references("groupB"))
	assert.Equal(t, sets.NewString("groupB"), r.referencedBy("uid1"))
	assert.Equal(t, sets.NewString(), r.referencedBy("uid2"))

	r.dereference("groupB", "uid1")
	assert.Equal(t, sets.NewString(), r.references("groupA"))
	assert.Equal(t, sets.NewString(), r.references("groupB"))
	assert.Equal(t, sets.NewString(), r.referencedBy("uid1"))
	assert.Equal(t, sets.NewString(), r.referencedBy("uid2"))

	assert.Empty(t, r.referenceToGroups)
	assert.Empty(t, r.groupToReferences)
}
