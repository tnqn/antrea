...
type networkPolicyController struct {
	*NetworkPolicyController
	...
	groupingController         *grouping.Controller
}
...
func newController(objects ...runtime.Object) (*fake.Clientset, *networkPolicyController) {
	...
	groupingController := grouping.NewController(groupEntityIndex,
...
func TestAddPod(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			npc.groupEntityIndex.AddPod(tt.addedPod)
...
func TestDeletePod(t *testing.T) {
	...
	npc.addClusterGroup(testCG)
	npc.groupEntityIndex.AddPod(p1)
	npc.groupEntityIndex.AddPod(p2)
	...
	npc.groupEntityIndex.DeletePod(p1)
	npc.syncAppliedToGroup(matchAppGID)
	...
	npc.groupEntityIndex.DeletePod(p2)
	npc.syncAddressGroup(addrGroup.Name)
	npc.syncInternalGroup(groupKey)
...
func TestAddNamespace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			npc.groupEntityIndex.AddNamespace(tt.addedNamespace)
...
func TestDeleteNamespace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			npc.groupEntityIndex.AddNamespace(tt.deletedNamespace)
			npc.groupEntityIndex.AddPod(p1)
			npc.groupEntityIndex.AddPod(p2)
			npc.groupEntityIndex.DeleteNamespace(tt.deletedNamespace)
			inGroupID := getNormalizedUID(toGroupSelector("", nil, &selectorIn, nil).NormalizedName)
			outGroupID := getNormalizedUID(toGroupSelector("", nil, &selectorOut, nil).NormalizedName)
			npc.syncAddressGroup(inGroupID)
			npc.syncAddressGroup(outGroupID)
...
func TestAddAndUpdateService(t *testing.T) {
	...
	npc.groupEntityIndex.AddPod(testPod1)
	npc.groupEntityIndex.AddPod(testPod2)
...
func TestDeleteService(t *testing.T) {
	...
	npc.cgStore.Add(testCG)
	npc.addClusterGroup(testCG)
	npc.groupEntityIndex.AddPod(testPod)
...
func TestGetAppliedToWorkloads(t *testing.T) {
	...
	_, c := newController()
	c.groupEntityIndex.AddPod(podA)
	c.groupEntityIndex.AddPod(podB)
	c.cgStore.Add(&cgA)
	c.cgStore.Add(&cgB)
