...
func (n *NetworkPolicyController) processAppliedTo(networkPolicyUID types.UID, namespace string, appliedTo []crdv1alpha1.NetworkPolicyPeer, appliedToGroupNamesSet sets.String) ([]string, error) {
	...
	for _, at := range appliedTo {
		...
		if at.Group != "" {
			var err error
			atg, err = n.processAppliedToGroupForNamespacedGroup(networkPolicyUID, namespace, at.Group)
			if err != nil {
...
func (n *NetworkPolicyController) processAppliedToGroupForNamespacedGroup(networkPolicyUID types.UID, namespace, groupName string) (string, error) {
