...
func (n *NetworkPolicyController) syncInternalGroup(key string) error {
	...
	if !found {
		...
		n.groupEntityIndex.DeleteGroup(clusterGroupType, key)
		return nil
	}
	...
	if grp.Selector != nil {
		n.groupEntityIndex.AddGroup(clusterGroupType, grp.Name, grp.Selector)
	} else {
		n.groupEntityIndex.DeleteGroup(clusterGroupType, grp.Name)
	}
...
func (n *NetworkPolicyController) GetAssociatedGroups(name, namespace string) ([]antreatypes.Group, error) {
	groups, _ := n.groupEntityIndex.GetGroupsForPod(namespace, name)
...
func (n *NetworkPolicyController) GetGroupMembers(cgName string) (controlplane.GroupMemberSet, error) {
	pods, externalEntities := n.groupEntityIndex.GetEntities(clusterGroupType, cgName)
