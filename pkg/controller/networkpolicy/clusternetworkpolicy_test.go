...
func TestProcessRefGroupOrClusterGroup(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actualAG, actualIPB := npc.processRefGroupOrClusterGroup("uid", tt.inputGroupName, tt.inputNamespace)
...
func TestProcessAppliedToGroupsForCGs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actualAG := npc.createAppliedToGroupForCG("uid", tt.inputCG)
...
func TestFilterPerNamespaceRuleACNPsByNSLabels(t *testing.T) {
	tests := []struct {
		...
		{
			name: "match multiple AppliedTo",
			nsLabels: map[string]string{
				"foo1": "bar1",
				"foo4": "bar4",
			},
			want: sets.NewString("cnp1", "cnp2"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
					...
					Ingress: []crdv1alpha1.Rule{
						{
							AppliedTo: []crdv1alpha1.NetworkPolicyPeer{
								{
									NamespaceSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"foo2": "bar2"}},
								},
							},
							From: []crdv1alpha1.NetworkPolicyPeer{
								{
