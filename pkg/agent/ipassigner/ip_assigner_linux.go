...
func (a *ipAssigner) loadIPAddresses() error {
	...
	addresses, err := netlink.AddrList(a.defaultAssignee.link, netlink.FAMILY_ALL)
	if err != nil {
		return err
	}
	for _, address := range addresses {
		a.assignedIPs[address.IP.String()] = nil
	}
	// Load IPs assigned to the vlan interfaces.
	for vlanID, vlanAssignee := range a.vlanAssignees {
		addresses, err := netlink.AddrList(vlanAssignee.link, netlink.FAMILY_ALL)
		if err != nil {
			return err
		}
		for _, address := range addresses {
			prefixLength, _ := address.Mask.Size()
			a.assignedIPs[address.IP.String()] = &crdv1b1.SubnetInfo{
...
func (a *ipAssigner) AssignIP(ip string, subnetInfo *crdv1b1.SubnetInfo, forceAdvertise bool) error {
...
func (a *ipAssigner) UnassignIP(ip string) error {
	...
	return a.unassign(parsedIP, subnetInfo)
}
...
func (a *ipAssigner) InitIPs(desired map[string]*crdv1b1.SubnetInfo) error {
	...
	for ip, desiredSubnetInfo := range desired {
		if err := a.AssignIP(ip, desiredSubnetInfo, true); err != nil {
			return err
		}
		staleIPs.Delete(ip)
	}
	for ip := range staleIPs {
		if err := a.UnassignIP(ip); err != nil {
			return err
		}
	}
	return nil
