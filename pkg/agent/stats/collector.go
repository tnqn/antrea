...
type statsCollection struct {
	...
	networkPolicyStats map[types.UID]*metricsv1alpha1.TrafficStats
	// antreaClusterNetworkPolicyStats is a mapping from Antrea ClusterNetworkPolicy UIDs to their traffic stats.
	antreaClusterNetworkPolicyStats map[types.UID]*metricsv1alpha1.TrafficStats
	// antreaNetworkPolicyStats is a mapping from Antrea NetworkPolicy UIDs to their traffic stats.
	antreaNetworkPolicyStats map[types.UID]*metricsv1alpha1.TrafficStats
}
...
func (m *Collector) collect() *statsCollection {
	...
	npStatsMap := map[types.UID]*metricsv1alpha1.TrafficStats{}
	acnpStatsMap := map[types.UID]*metricsv1alpha1.TrafficStats{}
	anpStatsMap := map[types.UID]*metricsv1alpha1.TrafficStats{}
	for ofID, ruleStats := range ruleStatsMap {
		...
		var statsMap map[types.UID]*metricsv1alpha1.TrafficStats
...
func (m *Collector) report(curStatsCollection *statsCollection) error {
	...
	_, err = antreaClient.ControlplaneV1alpha1().NodeStatsSummaries().Create(context.TODO(), summary, metav1.CreateOptions{})
	if err != nil {
		return err
	}
	return nil
...
func calculateDiff(curStatsMap, lastStatsMap map[types.UID]*metricsv1alpha1.TrafficStats) []cpv1alpha1.NetworkPolicyStats {
	if len(curStatsMap) == 0 {
		return nil
	}
	statsList := make([]cpv1alpha1.NetworkPolicyStats, 0, len(curStatsMap))
	for uid, curStats := range curStatsMap {
		var stats *metricsv1alpha1.TrafficStats
		...
		if !exists || curStats.Bytes < lastStats.Bytes {
			...
			stats = &metricsv1alpha1.TrafficStats{
		...
		policyStats := cpv1alpha1.NetworkPolicyStats{
			NetworkPolicy: cpv1alpha1.NetworkPolicyReference{UID: uid},
			TrafficStats:  *stats,
		}
