...
func TestSyncRoutes(t *testing.T) {
	...
	nodeRoute1 := &netlink.Route{Dst: getIPNet("192.168.1.0/24"), Gw: net.ParseIP("1.1.1.1")}
	nodeRoute2 := &netlink.Route{Dst: getIPNet("192.168.2.0/24"), Gw: net.ParseIP("1.1.1.2")}
	serviceRoute1 := &netlink.Route{Dst: getIPNet("169.254.0.253/32"), LinkIndex: 10}
	serviceRoute2 := &netlink.Route{Dst: getIPNet("169.254.0.252/32"), Gw: net.ParseIP("169.254.0.253")}
	...
	mockNetlink.EXPECT().RouteReplace(&netlink.Route{
		LinkIndex: 10,
		Dst:       getIPNet("192.168.0.0/24"),
		Src:       net.ParseIP("192.168.0.1"),
		Scope:     netlink.SCOPE_LINK,
	})
	mockNetlink.EXPECT().RouteReplace(&netlink.Route{
		LinkIndex: 10,
		Dst:       getIPNet("aabb:ccdd::/64"),
	...
	c := &Client{
		...
		nodeConfig: &config.NodeConfig{
			...
			PodIPv4CIDR:   getIPNet("192.168.0.0/24"),
			PodIPv6CIDR:   getIPNet("aabb:ccdd::/64"),
		},
	}
...
func TestSyncIPTables(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			nodeConfig: &config.NodeConfig{
				PodIPv4CIDR: getIPNet("172.16.10.0/24"),
...
func TestReconcile(t *testing.T) {
	...
	c := &Client{netlink: mockNetlink,
		...
		networkConfig: &config.NetworkConfig{},
		nodeConfig: &config.NodeConfig{
			PodIPv4CIDR:   getIPNet("192.168.10.0/24"),
			PodIPv6CIDR:   getIPNet("2001:ab03:cd04:55ee:100a::/80"),
	...
	mockNetlink.EXPECT().RouteDel(&netlink.Route{Dst: getIPNet("192.168.2.0/24")})
	mockNetlink.EXPECT().RouteDel(&netlink.Route{Dst: getIPNet("2001:ab03:cd04:55ee:1003::/80")})
	...
	mockNetlink.EXPECT().RouteListFiltered(netlink.FAMILY_V4, &netlink.Route{LinkIndex: 10}, netlink.RT_FILTER_OIF).Return([]netlink.Route{
		{Dst: getIPNet("192.168.10.0/24")},  // local podCIDR, should not be deleted.
		{Dst: getIPNet("192.168.1.0/24")},   // existing podCIDR, should not be deleted.
		{Dst: getIPNet("169.254.0.253/32")}, // service route, should not be deleted.
		{Dst: getIPNet("192.168.11.0/24")},  // non-existing podCIDR, should be deleted.
	}, nil)
	mockNetlink.EXPECT().RouteListFiltered(netlink.FAMILY_V6, &netlink.Route{LinkIndex: 10}, netlink.RT_FILTER_OIF).Return([]netlink.Route{
		{Dst: getIPNet("2001:ab03:cd04:55ee:100a::/80")},   // local podCIDR, should not be deleted.
		{Dst: getIPNet("2001:ab03:cd04:55ee:1001::1/128")}, // existing podCIDR, should not be deleted.
		{Dst: getIPNet("fc01::aabb:ccdd:eeff/128")},        // service route, should not be deleted.
		{Dst: getIPNet("2001:ab03:cd04:55ee:100b::/80")},   // non-existing podCIDR, should be deleted.
	}, nil)
	mockNetlink.EXPECT().RouteDel(&netlink.Route{Dst: getIPNet("192.168.11.0/24")})
	mockNetlink.EXPECT().RouteDel(&netlink.Route{Dst: getIPNet("2001:ab03:cd04:55ee:100b::/80")})
...
func TestAddRoutes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			podCIDR:  getIPNet("192.168.10.0/24"),
			nodeName: "node0",
		...
		},
		{
			...
			podCIDR:  getIPNet("2001:ab03:cd04:55ee:1001::/80"),
			nodeName: "node0",
			...
			expectedNetlinkCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				mockNetlink.RouteReplace(&netlink.Route{
					...
					Dst:       getIPNet("2001:ab03:cd04:55ee:1001::/80"),
					Scope:     netlink.SCOPE_LINK,
					LinkIndex: 11,
				})
		...
		},
		{
			...
			expectedNetlinkCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				mockNetlink.RouteReplace(&netlink.Route{
					Dst:       getIPNet("2001:ab03:cd04:55ee:1001::1/128"),
					LinkIndex: 10,
				})
				mockNetlink.RouteReplace(&netlink.Route{
...
func TestDeleteRoutes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			podCIDR: getIPNet("192.168.10.0/24"),
			existingNodeRoutes: map[string][]*netlink.Route{
				"192.168.10.0/24": {{Gw: net.ParseIP("172.16.10.3"), Dst: getIPNet("192.168.10.0/24")}},
				"192.168.11.0/24": {{Gw: net.ParseIP("172.16.10.4"), Dst: getIPNet("192.168.11.0/24")}},
			},
			expectedIPSetCalls: func(mockIPSet *ipsettest.MockInterfaceMockRecorder) {
				mockIPSet.DelEntry(antreaPodIPSet, "192.168.10.0/24")
			},
			expectedNetlinkCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				mockNetlink.RouteDel(&netlink.Route{Gw: net.ParseIP("172.16.10.3"), Dst: getIPNet("192.168.10.0/24")})
			},
		},
		{
			name:    "IPv6",
			podCIDR: getIPNet("2001:ab03:cd04:55ee:1001::/80"),
			existingNodeRoutes: map[string][]*netlink.Route{
				"2001:ab03:cd04:55ee:1001::/80": {{Gw: net.ParseIP("fe80::e643:4bff:fe44:1"), Dst: getIPNet("2001:ab03:cd04:55ee:1001::/80")}},
				"2001:ab03:cd04:55ee:1002::/80": {{Gw: net.ParseIP("fe80::e643:4bff:fe44:2"), Dst: getIPNet("2001:ab03:cd04:55ee:1002::/80")}},
			},
			existingNodeNeighbors: map[string]*netlink.Neigh{},
			expectedIPSetCalls: func(mockIPSet *ipsettest.MockInterfaceMockRecorder) {
			...
			expectedNetlinkCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				mockNetlink.RouteDel(&netlink.Route{Gw: net.ParseIP("fe80::e643:4bff:fe44:1"), Dst: getIPNet("2001:ab03:cd04:55ee:1001::/80")})
			},
		},
	}
	for _, tt := range tests {
...
func TestMigrateRoutesToGw(t *testing.T) {
	...
	mockNetlink.EXPECT().RouteList(link, netlink.FAMILY_V4).Return([]netlink.Route{
		{Gw: net.ParseIP("172.16.1.10"), Dst: getIPNet("192.168.10.0/24"), LinkIndex: 10},
	}, nil)
	mockNetlink.EXPECT().RouteList(link, netlink.FAMILY_V6).Return([]netlink.Route{
		{Gw: net.ParseIP("fe80::e643:4bff:fe44:1"), Dst: getIPNet("2001:ab03:cd04:55ee:1001::/80"), LinkIndex: 10},
	}, nil)
	mockNetlink.EXPECT().RouteReplace(&netlink.Route{Gw: net.ParseIP("172.16.1.10"), Dst: getIPNet("192.168.10.0/24"), LinkIndex: 11})
	mockNetlink.EXPECT().RouteReplace(&netlink.Route{Gw: net.ParseIP("fe80::e643:4bff:fe44:1"), Dst: getIPNet("2001:ab03:cd04:55ee:1001::/80"), LinkIndex: 11})
	...
	}{
		{
			...
			expectedCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				...
				mockNetlink.RouteList(gwLink, netlink.FAMILY_V4).Return([]netlink.Route{
					{Gw: net.ParseIP("172.16.1.10"), Dst: getIPNet("192.168.10.0/24"), LinkIndex: 11},
				}, nil)
				mockNetlink.RouteReplace(&netlink.Route{Gw: net.ParseIP("172.16.1.10"), Dst: getIPNet("192.168.10.0/24"), LinkIndex: 10})
			},
		},
		{
			...
			expectedCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				...
				mockNetlink.RouteDel(&netlink.Route{Gw: net.ParseIP("172.16.1.10"), Dst: getIPNet("192.168.10.0/24"), LinkIndex: 11})
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.expectedCalls(mockNetlink.EXPECT())
			c.UnMigrateRoutesFromGw(getIPNet(tt.route), tt.link)
		})
	}
}
	...
	}{
		{
			...
			expectedCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				...
				mockNetlink.RouteListFiltered(netlink.FAMILY_V4, &netlink.Route{LinkIndex: 10}, netlink.RT_FILTER_OIF).Return([]netlink.Route{
					{Dst: getIPNet("10.96.0.0/24"), Gw: config.VirtualServiceIPv4},
				}, nil)
		...
		},
		{
			...
			expectedCalls: func(mockNetlink *netlinktest.MockInterfaceMockRecorder) {
				...
				mockNetlink.RouteListFiltered(netlink.FAMILY_V6, &netlink.Route{LinkIndex: 10}, netlink.RT_FILTER_OIF).Return([]netlink.Route{
					{Dst: getIPNet("fd00:1234:5678:dead:beaf::/80"), Gw: config.VirtualServiceIPv6},
				}, nil)
				mockNetlink.RouteDel(&netlink.Route{
					Dst: getIPNet("fd00:1234:5678:dead:beaf::/80"), Gw: config.VirtualServiceIPv6,
				})
...
func TestAddLocalAntreaFlexibleIPAMPodRule(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			nodeConfig: &config.NodeConfig{
				PodIPv4CIDR: getIPNet("1.1.1.0/24"),
				PodIPv6CIDR: getIPNet("aabb::/64"),
			},
...
func TestAddAndDeleteNodeIP(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			multicastEnabled: true,
			networkConfig:    &config.NetworkConfig{TrafficEncapMode: config.TrafficEncapModeEncap},
			podCIDR:          getIPNet("192.168.0.0/24"),
