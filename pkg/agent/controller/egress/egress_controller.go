...
type EgressController struct {
	...
	// Whether it should read SubnetInfo of the ExternalIPPool or not.
...
func (c *EgressController) installPolicyRoute(ipState *egressIPState, subnetInfo *crdv1b1.SubnetInfo) error {
	...
	if !exists {
		...
		if err != nil {
			return fmt.Errorf("error allocating table for subnet %v: %w", subnetInfo, err)
		}
...
func (c *EgressController) syncEgress(egressName string) error {
	...
	if desiredNode == c.nodeName {
		...
		if err = c.ipAssigner.AssignIP(desiredEgressIP, subnetInfo, egress.Status.EgressNode != c.nodeName); err != nil {
			return err
		}
	} else {
