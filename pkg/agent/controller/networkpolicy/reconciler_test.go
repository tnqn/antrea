			...
			map[string]*lastRealized{
				"foo": {
					...
					CompletedRule: &CompletedRule{
						rule: &rule{Direction: v1beta1.DirectionIn, PolicyRef: &np1},
					},
				},
			},
			"unknown-rule-id",
		...
		},
		{
			...
			map[string]*lastRealized{
				"foo": {
					ofIDs: map[servicesKey]uint32{servicesKey1: 8, servicesKey2: 9},
					CompletedRule: &CompletedRule{
						rule: &rule{Direction: v1beta1.DirectionIn, PolicyPriority: &policyPriority, TierPriority: &tierPriority, PolicyRef: &cnp1},
					},
				},
			},
			"foo",
...
func TestReconcilerReconcile(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			"ingress-rule",
			&CompletedRule{
				rule:          &rule{ID: "ingress-rule", Direction: v1beta1.DirectionIn, Services: []v1beta1.Service{serviceTCP80, serviceTCP}, PolicyRef: &np1},
		...
		},
		{
			...
			&CompletedRule{
				rule:          &rule{ID: "ingress-rule", Direction: v1beta1.DirectionIn, PolicyRef: &np1},
				FromAddresses: addressGroup1,
				ToAddresses:   nil,
		...
		},
		{
			"egress-rule",
			&CompletedRule{
				rule:          &rule{ID: "egress-rule", Direction: v1beta1.DirectionOut, PolicyRef: &np1},
				FromAddresses: nil,
...
func TestReconcilerBatchReconcile(t *testing.T) {
	...
	completedRules := []*CompletedRule{
		...
		{
			rule: &rule{ID: "ingress-rule-no-ports", Direction: v1beta1.DirectionIn, Services: []v1beta1.Service{}, PolicyRef: &np1},
			Pods: appliedToGroup1,
		},
		{
			rule: &rule{ID: "ingress-rule-diff-named-port", Direction: v1beta1.DirectionIn, Services: []v1beta1.Service{serviceHTTP}, PolicyRef: &np1},
			Pods: appliedToGroupWithDiffContainerPort,
		},
		{
			rule:          &rule{ID: "egress-rule", Direction: v1beta1.DirectionOut, PolicyRef: &np1},
...
func TestReconcilerUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			&CompletedRule{
				rule:          &rule{ID: "ingress-rule", Direction: v1beta1.DirectionIn, PolicyPriority: &policyPriority, TierPriority: &tierPriority, PolicyRef: &cnp1},
				FromAddresses: addressGroup1,
				Pods:          appliedToGroup1,
			},
			&CompletedRule{
		...
		},
		{
			...
			&CompletedRule{
				rule:          &rule{ID: "ingress-rule", Direction: v1beta1.DirectionIn, PolicyPriority: &policyPriority, TierPriority: &tierPriority, Services: []v1beta1.Service{serviceHTTP}, PolicyRef: &cnp1},
				FromAddresses: addressGroup1,
				Pods:          appliedToGroupWithDiffContainerPort,
			},
			&CompletedRule{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			priority := gomock.Any()
			if !tt.originalRule.isAntreaPolicyRule() {
				priority = nil
			}
