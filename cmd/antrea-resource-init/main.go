...
func run(yamlResource string) error {
	...
	yamlData, err := ioutil.ReadFile(yamlResource)
	if err != nil {
		return err
	}
...
func applyOneYaml(ctx context.Context, client k8sclient.Client, obj *unstructured.Unstructured) error {
	...
	*opt.Force = true
	...
	if err := client.Patch(ctx, obj, k8sclient.Apply, owner, opt); err != nil {
		...
		return perrors.Wrapf(
			err,
			"Failed to patch object %s %s/%s",
			obj.GroupVersionKind(),
			obj.GetNamespace(),
			obj.GetName())
	}
...
func applyAPIServiceYaml(ctx context.Context, client k8sclient.Client, obj *unstructured.Unstructured) error {
	...
	if err != nil {
		return perrors.Wrapf(
			err,
			"Failed to cover object to json %s \"%s\"",
			obj.GroupVersionKind(),
			obj.GetName())
	}
...
func forEachObjectInYAML(
	...
	for {
		...
		case err := <-chanErr:
			...
			return perrors.Wrap(err, "received error while decoding yaml")
		}
	}
}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				chanErr <- perrors.Wrap(err, "failed to read yaml data")
				return
			}
			...
			if err := yaml.Unmarshal(buf, &obj.Object); err != nil {
				chanErr <- perrors.Wrap(err, "failed to unmarshal yaml data")
				return
			}
