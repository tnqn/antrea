...
func run(o *Options) error {
	...
	err = apiserver.CleanupDeprecatedWebhookConfigurations(client)
	if err != nil {
		return fmt.Errorf("failed to clean up the deprecated WebhookConfigurations: %v", err)
	}
	err = apiserver.CleanupDeprecatedCustomResourceDefinitions(apiExtensionClient)
	if err != nil {
		return fmt.Errorf("failed to clean up the deprecated CustomResourceDefinitions: %v", err)
	}
